...
const AmenityIcon = ({ name }: { name: string }) => {
  ...
  if (n.includes("breakfast") || n.includes("meal"))
      ...
      <div className="w-full md:w-3/5 p-6 md:p-8 flex flex-col justify-between space-y-4">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-[#6B7280]">
              ...
              <span className="text-xs font-bold capitalize tracking-wide">
                {hotel.location || "Luxury Location"}
              </span>
            </div>
          ...
          <p className="text-sm text-[#4A4A4A] leading-relaxed line-clamp-2 md:line-clamp-3">
            {hotel.description ||
              "Experience ultimate comfort and world-class hospitality at this hand-picked stay."}
          </p>
        </div>

        <div className="pt-4 border-t border-gray-50 flex items-center justify-between gap-4">
          <div className="flex flex-wrap gap-3">
            {(hotel.amenities || ["Wifi", "Breakfast", "Pool", "AC"]).map(
              (amenity, i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  <span className="text-[10px] font-bold capitalize tracking-tighter hidden sm:inline">
                    {amenity}
                  </span>
                </div>
              ),
