  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-slate-400 font-bold capitalize tracking-widest text-xs">
          Loading Knowledge Quest...
        </p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="space-y-3 flex-grow">
                {q.options.map((opt, i) => {
                  ...
                  let buttonClass =
                    "w-full text-left p-4 rounded-2xl border transition-all duration-300 font-medium ";
                  if (showResult) {
                    if (isCorrect)
                      buttonClass +=
              ...
              <div className="mt-8">
                {showResults[q._id] ? (
                  <div
                    className={`p-4 rounded-2xl text-center font-bold text-sm capitalize tracking-widest ${selectedAnswers[q._id] === q.answer ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                  >
