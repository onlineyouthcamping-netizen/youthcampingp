      ...
      <div className="fixed inset-0 bg-white z-[999] flex flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-bold text-slate-900 capitalize tracking-tighter">
              Proposal Expired
            </h1>
            <p className="text-slate-500 font-medium">
              This quotation for{" "}
              <span className="text-slate-900 font-bold">{q.tripTitle}</span> is
              no longer valid. Please contact your travel expert for an updated
              proposal.
            </p>
          </div>
          <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex items-center gap-4 text-left">
            <img
              src={normalizeImageUrl(q.expert?.photo)}
              className="w-16 h-16 rounded-2xl object-cover shadow-md"
            />
            <div>
              <p className="text-[10px] font-bold text-slate-400 capitalize tracking-widest leading-none mb-1">
                Your Expert
              </p>
          ...
          <a
            href={`https://wa.me/${q.expert?.whatsapp?.replace(/\D/g, "")}?text=Hi ${q.expert?.name}, my quote for ${q.tripTitle} has expired. Can you please refresh it?`}
            className="block"
          >
    ...
    <div className="bg-[#f8fafc] min-h-screen font-sans selection:bg-orange-100 selection:text-orange-900 pb-24">
      ...
      <section className="relative h-[60vh] md:h-[65vh] w-full overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 md:p-12 max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-4">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  {new Date(q.travelDates.from).toLocaleDateString("en-US", {
                    month: "short",
                    day: "numeric",
                  })}{" "}
                  -{" "}
                  {new Date(q.travelDates.to).toLocaleDateString("en-US", {
                    month: "short",
                    day: "numeric",
                    year: "numeric",
                  })}
            ...
            >
              <p className="text-[10px] font-bold text-white/60 capitalize tracking-widest mb-1">
                Final Price
              </p>
              ...
              {q.discount > 0 && (
                <p className="text-xs font-bold text-orange-400 line-through opacity-80">
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          >
            ...
            <p className="text-slate-600 text-base md:text-lg leading-relaxed font-medium">
              {q.overview ||
                "Embark on an unforgettable journey curated just for you. Experience the perfect blend of luxury, comfort, and exploration as we take you through the hidden gems and iconic landmarks of your destination."}
            </p>
          </motion.div>
              ...
              {q.itinerary?.map((day, i) => (
                ...
                >
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`text-slate-400 transition-transform duration-300 ${expandedDays.includes(i) ? "rotate-180" : ""}`}
                    />
                  </button>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                              {day.photos.map((photo, pIdx) => {
                                ...
                                return (
                                  <div
                                    key={pIdx}
                                    className="aspect-video rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-slate-100"
                                  >
                                    <img
            ...
            >
              ...
              <ul className="space-y-4">
                {q.inclusions?.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-3 text-slate-600 font-bold text-sm"
                  >
            ...
            >
              ...
              <ul className="space-y-4">
                {q.exclusions?.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-3 text-slate-500 font-bold text-sm"
                  >
          ...
          <div className="sticky top-24 space-y-8">
            ...
            >
              ...
              <div className="relative z-10 space-y-8">
                <div>
                  <p className="text-[10px] font-bold text-orange-400 capitalize tracking-[0.2em] mb-2">
                    Package Investment
                  </p>
                  ...
                  {q.discount > 0 && (
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <span className="bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-lg">
                        Save ₹ {q.discount?.toLocaleString()}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <Button
                    variant="outline"
                    className="w-full h-14 border-white/10 hover:bg-white/5 rounded-2xl font-bold capitalize text-xs tracking-widest transition-all text-white"
                  >
            ...
            <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-xl shadow-slate-200/50 space-y-6">
              <h4 className="text-sm font-bold text-slate-400 capitalize tracking-widest text-center border-b pb-4 mb-4">
                Your Travel Expert
              </h4>
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="w-full grid grid-cols-2 gap-4 pt-4">
                  <a
                    href={whatsappLink}
                    target="_blank"
                    className="bg-emerald-500 text-white p-4 rounded-2xl flex flex-col items-center gap-2 hover:scale-[1.02] transition-transform"
                  >
                  ...
                  <a
                    href={`tel:${q.expert?.whatsapp}`}
                    className="bg-slate-900 text-white p-4 rounded-2xl flex flex-col items-center gap-2 hover:scale-[1.02] transition-transform"
                  >
                    <Phone size={20} />
                    <span className="text-[10px] font-bold capitalize tracking-widest">
                      Call Now
      ...
      >
        <a href={whatsappLink} target="_blank" className="flex-1">
          <Button
            variant="outline"
            className="w-full h-12 rounded-xl border-emerald-100 text-emerald-600 font-bold capitalize text-[10px] tracking-widest"
          >
