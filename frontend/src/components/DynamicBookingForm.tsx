...
const DynamicBookingForm: React.FC<DynamicBookingFormProps> = ({ formId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/dynamic-forms/${formId}/submit`,
        {
          method: "POST",
      ...
      if (result.success) {
        ...
        setError(
          result.message || "Submission failed. Please check your data.",
        );
      }
    } catch (err) {
  ...
  if (success) {
    return (
      <div className="max-w-2xl mx-auto p-12 bg-white rounded-[3rem] shadow-2xl text-center border-t-8 border-primary animate-in fade-in duration-500">
        ...
        <h2 className="text-4xl font-bold capitalize tracking-tighter mb-4">
          Booking Saved!
        </h2>
        <p className="text-muted-foreground mb-8 font-medium">
          Your data has been successfully pushed to the Master Sheet.
        </p>
        <button
          onClick={() => setSuccess(false)}
          className="w-full bg-primary py-5 rounded-2xl text-white font-bold capitalize tracking-widest text-xs hover:bg-primary/90 transition-all shadow-xl shadow-primary/20"
        >
          Create New Booking
        </button>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pt-4 pb-32 md:pb-12">
      {" "}
      {/* 6. MOBILE FIX: Bottom spacing */}
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <Section title="Travel Info">
          <Select
            ...
            onChange={(v) => handleChange("trainClass", v)}
            options={["3AC", "SL"]}
          />
          <Select
            label="Ticket Status"
            ...
            onChange={(v) => handleChange("ticketStatus", v)}
        ...
        <Section title="Payment Info">
          <Input
            ...
            onChange={(v) => handleChange("advancePayment", v)}
            placeholder="₹ Amount"
            type="number"
          />
          <Input
            label="Advance Txn ID"
            icon={<Hash size={18} />}
            value={formData.advanceTransactionId}
            onChange={(v) => handleChange("advanceTransactionId", v)}
            placeholder="TXN123..."
          />
          <Input
            label="Advance Verified By"
            icon={<User size={18} />}
            value={formData.advanceVerifiedBy}
            onChange={(v) => handleChange("advanceVerifiedBy", v)}
          ...
          <Input
            ...
            onChange={(v) => handleChange("advancePaymentDate", v)}
            type="date"
          />
          ...
          <Input
            ...
            onChange={(v) => handleChange("remainingPayment", v)}
            placeholder="₹ Amount"
            type="number"
          />
          <Input
            label="Remaining Txn ID"
            icon={<Hash size={18} />}
            value={formData.remainingTransactionId}
            onChange={(v) => handleChange("remainingTransactionId", v)}
            placeholder="TXN456..."
          />
          <Input
            label="Remaining Verified By"
            icon={<User size={18} />}
            value={formData.remainingVerifiedBy}
            onChange={(v) => handleChange("remainingVerifiedBy", v)}
            placeholder="Staff Name"
          />
          <Input
            ...
            onChange={(v) => handleChange("remainingPaymentDate", v)}
            type="date"
          />
        </Section>
        ...
        <Section title="Other">
          <div className="md:col-span-2 lg:col-span-3">
            <label className="text-[10px] font-bold capitalize tracking-widest text-muted-foreground mb-2 block">
              Remark
            </label>
            <textarea
        ...
        <div className="bg-zinc-900 rounded-[2.5rem] p-8 text-white flex flex-col md:flex-row justify-between items-center gap-8 shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold capitalize tracking-widest">
                Apply 5% GST
              </p>
          ...
          <div className="flex gap-8 items-center text-right">
            ...
            {applyGST && (
              <div>
                <p className="text-[10px] font-bold capitalize tracking-widest text-primary">
                  GST (5%)
                </p>
        ...
        >
          {loading ? (
            ...
            <>
              <Send size={18} /> Sync to Master Sheet
            </>
          )}
        </button>
      </form>
...
const Section = ({
  ...
  <div className="space-y-4">
    <div className="flex items-center gap-4">
      <h3 className="text-xs font-bold capitalize tracking-[0.3em] text-zinc-400 whitespace-nowrap">
        {title}
      </h3>
