...
const mockHotels = [
  { id: "HTL-1", name: "Apple Blossom", city: "Sangla", category: "Standard", rating: "★★★★", roomsAvailable: { Twin: 10, Triple: 8, Quad: 4 }, address: "Main Bazaar, Sangla, HP" },
  { id: "HTL-2", name: "Hotel Snow View", city: "Shimla", category: "Deluxe", rating: "★★★★★", roomsAvailable: { Twin: 15, Triple: 10, Quad: 5 }, address: "Mall Road, Shimla, HP" },
  { id: "HTL-3", name: "Mehak Resort", city: "Sangla", category: "Luxury", rating: "★★★★", roomsAvailable: { Twin: 12, Triple: 6, Quad: 2 }, address: "Baspa Valley, Sangla, HP" },
  { id: "HTL-4", name: "Spiti Siddharth", city: "Kaza", category: "Standard", rating: "★★★★", roomsAvailable: { Twin: 8, Triple: 6, Quad: 4 }, address: "Old Kaza, HP" },
  { id: "HTL-5", name: "Mountain Vista", city: "Tabo", category: "Deluxe", rating: "★★★★★", roomsAvailable: { Twin: 14, Triple: 8, Quad: 4 }, address: "Tabo Monastery Road, HP" },
];

const mockHotelVendors = [
  { id: "HV-1", hotelId: "HTL-1", vendorId: "VND-1", vendorName: "Direct Hotel", negotiatedRate: 3200, lastUsedRate: 3200, paymentTerms: "100% at Check-in", outstandingAmount: 0, defaultVendor: true, commission: "0%", active: true },
  { id: "HV-2", hotelId: "HTL-1", vendorId: "VND-2", vendorName: "Mountain Hospitality", negotiatedRate: 2950, lastUsedRate: 3000, paymentTerms: "50% Advance, 50% Post-Trip", outstandingAmount: 45000, defaultVendor: false, commission: "8%", active: true },
  { id: "HV-3", hotelId: "HTL-1", vendorId: "VND-3", vendorName: "XYZ Travels", negotiatedRate: 3100, lastUsedRate: 3150, paymentTerms: "7 Days Credit", outstandingAmount: 12000, defaultVendor: false, commission: "5%", active: true },
  { id: "HV-4", hotelId: "HTL-2", vendorId: "VND-2", vendorName: "Mountain Hospitality", negotiatedRate: 4200, lastUsedRate: 4200, paymentTerms: "50% Advance", outstandingAmount: 45000, defaultVendor: true, commission: "10%", active: true },
  { id: "HV-5", hotelId: "HTL-3", vendorId: "VND-2", vendorName: "Mountain Hospitality", negotiatedRate: 3600, lastUsedRate: 3600, paymentTerms: "50% Advance", outstandingAmount: 45000, defaultVendor: true, commission: "10%", active: true },
  { id: "HV-6", hotelId: "HTL-4", vendorId: "VND-4", vendorName: "Spiti Valley Escapes", negotiatedRate: 2800, lastUsedRate: 2800, paymentTerms: "100% Advance", outstandingAmount: 0, defaultVendor: true, commission: "5%", active: true },
  { id: "HV-7", hotelId: "HTL-5", vendorId: "VND-2", vendorName: "Mountain Hospitality", negotiatedRate: 3800, lastUsedRate: 3800, paymentTerms: "50% Advance", outstandingAmount: 45000, defaultVendor: true, commission: "10%", active: true },
  { id: "HV-8", hotelId: "HTL-5", vendorId: "VND-5", vendorName: "ABC Travels", negotiatedRate: 3900, lastUsedRate: 3950, paymentTerms: "15 Days Credit", outstandingAmount: 25000, defaultVendor: false, commission: "7%", active: true },
];
...
let mockDepartureStays = [
  {
    ...
    timeline: [
      ...
      { step: "Rate Finalized", completed: true, timestamp: "01 Aug 2026 11:30 AM" },
      { step: "Voucher Sent", completed: true, timestamp: "02 Aug 2026 02:15 PM" },
      { step: "Hotel Confirmed", completed: true, timestamp: "02 Aug 2026 04:00 PM" },
  ...
  },
  {
    ...
    timeline: [
      { step: "Draft", completed: true, timestamp: "01 Aug 2026 10:00 AM" },
      { step: "Rate Finalized", completed: true, timestamp: "01 Aug 2026 11:45 AM" },
      { step: "Voucher Sent", completed: true, timestamp: "02 Aug 2026 02:30 PM" },
      { step: "Hotel Confirmed", completed: true, timestamp: "02 Aug 2026 04:30 PM" },
      { step: "Checked In", completed: false },
      { step: "Checked Out", completed: false },
      { step: "Invoice Received", completed: false },
      { step: "Paid", completed: false },
...
router.get('/admin/hotels', optionalAuthenticate, async (req, res) => {
  ...
  const filtered = city ? mockHotels.filter(h => h.city.toLowerCase() === city.toLowerCase()) : mockHotels;
...
router.get('/admin/departure-stays/:departureId', optionalAuthenticate, async (req, res) => {
  const { departureId } = req.params;
  const stays = mockDepartureStays.filter(s => s.departureId === departureId || departureId === 'all');
  res.json({ success: true, data: stays });
});

// 12. Create/Assign a new Departure Stay (from Step 4 Wizard)
router.post('/admin/departure-stays', optionalAuthenticate, async (req, res) => {
  try {
    const newStay = {
      id: `STAY-${Date.now()}`,
      ...
      timeline: [
        { step: "Draft", completed: true, timestamp: new Date().toLocaleString() },
    ...
    res.status(500).json({ success: false, error: "Failed to create stay assignment" });
  }
});

// 13. Update Stay Status or Details
router.put('/admin/departure-stays/:id', optionalAuthenticate, async (req, res) => {
  const { id } = req.params;
  const idx = mockDepartureStays.findIndex(s => s.id === id);
  if (idx === -1) return res.status(404).json({ success: false, error: "Stay not found" });
