  ...
  async (req, res, next) => {
    try {
      ...
      if (existing) {
        ...
        if (
          req.user?.role === "sales" &&
          existing.salesAdminId !== req.user.id
        ) {
          return res
            .status(404)
            .json({ success: false, message: "Quotation not found" });
        ...
        if (
          req.user?.role === "sales" &&
          body.salesAdminId !== undefined &&
          body.salesAdminId !== existing.salesAdminId
        ) {
          return res
            .status(403)
            .json({
              success: false,
              message: "Sales users cannot modify quotation ownership",
            });
        }
  ...
  async (req, res, next) => {
    try {
      ...
      const newExpiry = new Date(
        new Date().getTime() + (hours || 48) * 60 * 60 * 1000,
      );
