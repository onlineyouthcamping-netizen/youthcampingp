  ...
  async (req, res, next) => {
    try {
      ...
      const vendors = rawVendors.map((tv) => ({
        ...
        role: tv.role,
        contactName:
          tv.contactName || tv.vendor?.contactPerson || tv.vendor?.name || "",
    ...
    try {
      ...
      const tripVendor = await prisma.tripVendor.create({
        data: {
          ...
          contactName:
            contactName ||
            (vDetails ? vDetails.contactPerson || vDetails.name : null),
  ...
  async (req, res, next) => {
    try {
      ...
      const tripVendor = await prisma.tripVendor.findFirst({
        where: {
          tripId,
          OR: [{ id: vendorId }, { vendorId: vendorId }],
        },
        include: { vendor: true },
      });
      ...
      if (!tripVendor) {
        return res
          .status(404)
          .json({ success: false, message: "Trip vendor link not found" });
      }
