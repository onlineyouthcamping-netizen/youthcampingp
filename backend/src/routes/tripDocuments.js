...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    ...
    const uploadDir = path.join(__dirname, '../../public/uploads/trips', tripId);
...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedExts.includes(ext) || allowedMimeTypes.includes(file.mimetype)) {
    ...
    cb(new Error('Invalid file type. Only .pdf, .png, .jpg, .jpeg, and .docx files are allowed.'));
  }
};
 ...
 */
router.post('/:tripId/documents/upload', authenticate, requirePermission('edit_trip'), (req, res, next) => {
  upload.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      try {
        ...
        const secondaryFilePath = path.join(__dirname, '../../uploads/trips', tripId, storedFileName);
        if (fs.existsSync(primaryFilePath) && !fs.existsSync(secondaryFilePath)) {
 ...
 */
router.get('/:tripId/documents', authenticate, requirePermission('view_trip'), async (req, res, next) => {
  try {
    const { tripId } = req.params;
 ...
 * Auto-log deletion
 */
router.delete('/:tripId/documents/:docId', authenticate, requirePermission('edit_trip'), async (req, res, next) => {
  try {
    ...
    if (doc.fileUrl && doc.fileUrl.startsWith('/uploads/')) {
      ...
      if (fs.existsSync(rootUploadPath)) {
        try {
          ...
          console.error('Failed to delete file from disk:', rootUploadPath, e);
        }
      }
    }
    ...
    await prisma.tripActivityLog.create({
      data: {
        ...
        section: 'documents',
        itemId: docId,
        changes: { fileName: doc.name || doc.fileName, category: doc.category },
        performedBy: req.user.id
      }
    });
