 ...
 */
router.get('/:tripId/knowledge/search', authenticate, requirePermission('view_trip'), async (req, res, next) => {
  try {
 ...
 */
router.get('/:tripId/activity', authenticate, requirePermission('view_trip'), async (req, res, next) => {
  try {
    const { tripId } = req.params;
 ...
 */
router.get('/:tripId/knowledge/:section', authenticate, requirePermission('view_trip'), async (req, res, next) => {
  try {
 ...
 */
router.post('/:tripId/knowledge/:section', authenticate, requirePermission('edit_trip'), async (req, res, next) => {
  try {
    const { tripId, section } = req.params;
    ...
    await prisma.tripActivityLog.create({
      data: {
        ...
        changes: { title, contentType: createdItem.contentType, status: createdItem.status },
 ...
 * Return: updated item
 */
router.put('/:tripId/knowledge/item/:id', authenticate, requirePermission('edit_trip'), async (req, res, next) => {
  try {
    ...
    await prisma.tripActivityLog.create({
      data: {
        ...
        itemId: id,
        changes: {
          before: { title: existing.title, body: existing.body, status: existing.status, version: existing.version },
          after: { title: updatedItem.title, body: updatedItem.body, status: updatedItem.status, version: updatedItem.version }
        },
        performedBy: req.user.id
      }
    });
...
router.put('/:tripId/knowledge/:id', authenticate, requirePermission('edit_trip'), async (req, res, next) => {
  try {
    const { tripId, id } = req.params;
    const { title, body, contentType, data, status } = req.body;
 ...
 */
router.delete('/:tripId/knowledge/:id', authenticate, requirePermission('edit_trip'), async (req, res, next) => {
  try {
    const { tripId, id } = req.params;
