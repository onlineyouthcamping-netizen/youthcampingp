...
const getConfig = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error getting design config" });
  }
};
...
const getMergedConfig = async (req, res) => {
  try {
    ...
    if (scope.startsWith("section:")) {
      ...
      const sectionConfigRecord = await prisma.designConfig.findUnique({
        where: {
          tenantId_scope_status: {
            tenantId: "default",
            scope: `section:${pageSlug}:${sectionId}`,
            status: "published",
          },
        },
      });
    ...
    res
      .status(500)
      .json({ success: false, message: "Error getting merged design config" });
  }
};
  ...
  try {
    ...
    await prisma.designVersion.create({
      data: {
        ...
        affectedPages: scope.startsWith("page:")
          ? [scope.split(":")[1]]
          : scope.startsWith("section:")
            ? [scope.split(":")[1]]
            : ["global"],
        affectedSections: scope.startsWith("section:")
          ? [scope.split(":")[2]]
          : [],
      },
    });
    ...
    res
      .status(500)
      .json({ success: false, message: "Error saving draft design config" });
  }
};
...
const publishConfig = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error publishing design config" });
  }
};
...
const getVersions = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error getting version history" });
  }
};
...
const restoreVersion = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error restoring design version" });
  }
};
...
const compareVersions = async (req, res) => {
  try {
    ...
    if (!versionA) {
      return res
        .status(404)
        .json({ success: false, message: "Version A not found" });
    }

    let versionB = null;
    if (compareTo) {
      versionB = await prisma.designVersion.findUnique({
        where: { id: compareTo },
      });
    } else {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error comparing versions" });
  }
};
