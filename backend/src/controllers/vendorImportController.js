  ...
  if (workbook.Sheets["MKA Trip Details"]) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (
        ...
        !hotelName.includes("Tempo") &&
        !hotelName.includes("Pick") &&
        !hotelName.includes("Drop") &&
        i < 12
      ) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (
        vehicleDesc &&
        typeof vehicleDesc === "string" &&
        (vehicleDesc.includes("Tempo") ||
          vehicleDesc.includes("Scorpio") ||
          vehicleDesc.includes("Innova"))
      ) {
        ...
        if (!isNaN(cost)) {
          results.transport.push({
            ...
            sellableSeats: vehicleDesc.includes("12")
              ? 12
              : vehicleDesc.includes("17")
                ? 14
                : 17,
            tripCode: "MKA",
            sourceSheet: "MKA Trip Details",
  ...
  if (workbook.Sheets["Spiti Trip Details"]) {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (
        hotelName &&
        typeof hotelName === "string" &&
        !hotelName.includes("Route") &&
        !hotelName.includes("Guide") &&
        !hotelName.includes("Extra")
      ) {
        const phone = normalizePhone(row[1]);
    ...
    for (let i = 4; i < 11; i++) {
      ...
      if (vehicle && !isNaN(cost)) {
        results.transport.push({
          ...
          totalVehicleCost: cost,
          sellableSeats: String(vehicle).includes("20")
            ? 17
            : String(vehicle).includes("17")
              ? 14
              : 12,
          tripCode: "SMDD",
          sourceSheet: "SMDD Trip Details",
...
exports.confirmImport = async (req, res) => {
  try {
    ...
    for (const h of hotels) {
      ...
      const trip = await getOrCreateTripBySheet(
        prisma,
        h.sourceSheet,
        tenantId,
      );
      ...
      let vendor = await prisma.opsVendor.findFirst({
        where: {
          ...
          location: h.city
            ? { equals: h.city, mode: "insensitive" }
            : undefined,
        },
      });
    ...
    return res
      ...
      .json({
        success: false,
        message: "Transaction rolled back: Import failed",
      });
  }
};
