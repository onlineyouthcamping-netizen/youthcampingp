...
async function generateReceiptNumber(tenantId) {
  ...
  const count = await prisma.stationPaymentCollection.count({ where: { tenantId } });
...
async function sendReceiptEmail(collection, booking, tripName, adminName) {
  ...
  try {
    ...
    const verificationNote = isUpi && collection.upiVerificationStatus === 'PENDING_VERIFICATION'
...
exports.getDashboard = async (req, res) => {
  try {
    ...
    const bookings = await prisma.booking.findMany({
      where: { tenantId, tripId, departureDate: depDate, status: { not: 'cancelled' } },
      include: {
        stationPayments: { where: { collectionStatus: { not: 'CANCELLED' } }, include: { receivingAccount: true, collectedBy: { select: { id: true, name: true } } } },
        salesAdmin: { select: { id: true, name: true } }
      }
    });

    let totalPackageAmount=0, totalPreviouslyPaid=0, totalCashCollected=0, totalUpiCollected=0, totalVerifiedUpi=0, totalUnverifiedUpi=0;
    let fullyPaid=0, partiallyPaid=0, unpaid=0;
    const collectorCashMap = {}, collectorUpiMap = {};
    const bookingRows = [];

    for (const bk of bookings) {
      const finalAmount = bk.totalAmount || bk.amount || 0;
      const currentAdvancePaid = bk.advancePaid || 0;
      const cashPmts = bk.stationPayments.filter(p => p.paymentMode === 'CASH' && p.collectionStatus === 'COLLECTED');
      const upiPmts = bk.stationPayments.filter(p => p.paymentMode === 'UPI' && p.collectionStatus === 'COLLECTED');
      const verifiedUpi = upiPmts.filter(p => p.upiVerificationStatus === 'VERIFIED');
      const pendingUpi = upiPmts.filter(p => p.upiVerificationStatus === 'PENDING_VERIFICATION');
      ...
      for (const p of cashPmts) {
        ...
        if (!collectorCashMap[cid]) collectorCashMap[cid] = { id: cid, name: p.collectedBy?.name || cid, cash: 0 };
        collectorCashMap[cid].cash += p.amount;
      }
      for (const p of upiPmts) {
        const cid = p.collectedByAdminId;
        if (!collectorUpiMap[cid]) collectorUpiMap[cid] = { id: cid, name: p.collectedBy?.name || cid, upi: 0, verified: 0 };
        collectorUpiMap[cid].upi += p.amount;
        if (p.upiVerificationStatus === 'VERIFIED') collectorUpiMap[cid].verified += p.amount;
      }
      ...
      bookingRows.push({
        ...
        paymentStatus: bk.paymentStatus, collectionStatus: collStat, stationPayments: bk.stationPayments
      });
    }

    const handovers = await prisma.stationCashHandover.findMany({
      where: { tenantId, tripId, departureDate: depDate },
      include: { collector: { select: { id: true, name: true } }, handoverRecipient: { select: { id: true, name: true } } }
    });
    const cashHandedOver = handovers.filter(h => ['HANDED_OVER','CONFIRMED','RECONCILED'].includes(h.handoverStatus)).reduce((s,h) => s+h.amountHandedOver, 0);
    const cashReconciled = handovers.filter(h => h.handoverStatus === 'RECONCILED').reduce((s,h) => s+h.amountHandedOver, 0);

    const stats = {
      totalBookings: bookings.length, totalPackageAmount, totalPreviouslyPaid,
      totalCashCollected, totalUpiCollected, totalVerifiedUpi, totalUnverifiedUpi,
      totalStationCollection: totalCashCollected + totalVerifiedUpi,
      grandTotalReceived: totalPreviouslyPaid + totalCashCollected + totalVerifiedUpi,
      totalRemaining: totalPackageAmount - (totalPreviouslyPaid + totalCashCollected + totalVerifiedUpi),
      fullyPaid, partiallyPaid, unpaid,
      cashAwaitingHandover: totalCashCollected - cashHandedOver, cashHandedOver, cashReconciled,
    ...
    if (station) filtered = filtered.filter(r => r.stationPayments.some(p => p.station?.toLowerCase().includes(station.toLowerCase())));
    if (collectionStatus) filtered = filtered.filter(r => r.collectionStatus === collectionStatus);
    if (paymentMode) filtered = filtered.filter(r => r.stationPayments.some(p => p.paymentMode === paymentMode));
    if (salespersonId) filtered = filtered.filter(r => r.salespersonId === salespersonId);
    if (collectorId) filtered = filtered.filter(r => r.stationPayments.some(p => p.collectedByAdminId === collectorId));
    if (search) {
      const q = search.toLowerCase();
      filtered = filtered.filter(r => r.bookingId?.toLowerCase().includes(q) || r.name?.toLowerCase().includes(q) || r.phone?.includes(q) || r.email?.toLowerCase().includes(q));
    }
...
exports.collect = async (req, res) => {
  try {
    ...
    if (parsedAmount > currentRemaining + 0.01)
      return res.status(400).json({ success: false, message: `Amount ₹${parsedAmount} exceeds remaining balance of ${fmt(currentRemaining)}` });

    if (paymentMode === 'UPI' && utrNumber) {
      const dupUtr = await prisma.stationPaymentCollection.findFirst({ where: { tenantId, utrNumber } });
      if (dupUtr) return res.status(409).json({ success: false, message: `UTR ${utrNumber} already recorded. Duplicate rejected.` });
    }
    if (paymentMode === 'CASH') {
    ...
    const newRemaining = isCash ? Math.max(0, finalAmount - newTotalPaid) : currentRemaining;
    const newPaymentStatus = isCash ? (newRemaining <= 0 ? 'PAID' : 'PARTIAL') : booking.paymentStatus;

    const collection = await prisma.$transaction(async (tx) => {
      const col = await tx.stationPaymentCollection.create({
        data: {
          tenantId, receiptNumber, bookingId, tripId,
      ...
      await tx.accountingEntry.create({
        data: {
          tenantId, bookingId, amount: parsedAmount,
      ...
      if (isCash) {
        await tx.booking.update({
          where: { bookingId },
          data: { advancePaid: newTotalPaid, remainingAmount: newRemaining, paymentStatus: newPaymentStatus, payment_status: newPaymentStatus === 'PAID' ? 'paid' : 'partial' }
        });
      }
    ...
    return res.status(201).json({
      ...
      message: isCash ? `Cash ${fmt(parsedAmount)} recorded. Receipt: ${receiptNumber}` : `UPI ${fmt(parsedAmount)} recorded. Pending Finance verification. Receipt: ${receiptNumber}`
    });
  } catch (err) {
    console.error('[StationPayment] collect error:', err);
    if (err.code === 'P2002') return res.status(409).json({ success: false, message: 'Duplicate UTR number. This transaction was already recorded.' });
...
exports.getOne = async (req, res) => {
  try {
    const record = await prisma.stationPaymentCollection.findUnique({ where: { id: req.params.id }, include: { receivingAccount: true, collectedBy: { select: { id: true, name: true } }, verifiedBy: { select: { id: true, name: true } }, reversedBy: { select: { id: true, name: true } }, handover: true } });
...
exports.cancel = async (req, res) => {
  try {
    ...
    if (!reversalReason) return res.status(400).json({ success: false, message: 'reversalReason is required' });
    const record = await prisma.stationPaymentCollection.findUnique({ where: { id: req.params.id } });
    if (!record) return res.status(404).json({ success: false, message: 'Record not found' });
    if (record.isReversed) return res.status(409).json({ success: false, message: 'Already reversed' });
...
exports.verifyUpi = async (req, res) => {
  try {
    ...
    const booking = await prisma.booking.findUnique({ where: { bookingId: record.bookingId } });
    const finalAmount = booking ? (booking.totalAmount || booking.amount || 0) : 0;
...
exports.getReports = async (req, res) => {
  try {
    ...
    if (from && to) where.collectedAt = { gte: new Date(from), lte: new Date(to) };

    const collections = await prisma.stationPaymentCollection.findMany({ where, include: { receivingAccount: { select: { accountName: true, ownershipType: true } }, collectedBy: { select: { name: true } } }, orderBy: { collectedAt: 'desc' } });

    let cashTotal=0, upiTotal=0, upiVerified=0, upiPending=0;
    const byAccount = {}, byCollector = {};
    for (const c of collections) {
      if (c.paymentMode === 'CASH') { cashTotal += c.amount; }
      else { upiTotal += c.amount; if (c.upiVerificationStatus==='VERIFIED') upiVerified+=c.amount; else if (c.upiVerificationStatus==='PENDING_VERIFICATION') upiPending+=c.amount; }
      if (c.paymentMode === 'UPI') {
        const k = c.receivingAccountId || 'unknown';
        if (!byAccount[k]) byAccount[k] = { accountName: c.receivingAccount?.accountName||'Unknown', ownershipType: c.receivingAccount?.ownershipType, amount: 0, verified: 0, pending: 0 };
        byAccount[k].amount += c.amount;
        if (c.upiVerificationStatus==='VERIFIED') byAccount[k].verified += c.amount;
        if (c.upiVerificationStatus==='PENDING_VERIFICATION') byAccount[k].pending += c.amount;
      }
      const ck = c.collectedByAdminId;
      if (!byCollector[ck]) byCollector[ck] = { name: c.collectedBy?.name||ck, cash: 0, upi: 0, verifiedUpi: 0 };
      if (c.paymentMode==='CASH') byCollector[ck].cash += c.amount; else { byCollector[ck].upi += c.amount; if (c.upiVerificationStatus==='VERIFIED') byCollector[ck].verifiedUpi += c.amount; }
    }
    return res.json({ success: true, data: { summary: { cashTotal, upiTotal, upiVerified, upiPending, grandTotal: cashTotal + upiVerified }, byAccount: Object.values(byAccount), byCollector: Object.values(byCollector), collections } });
  } catch (err) { return res.status(500).json({ success: false, message: 'Server error' }); }
};
