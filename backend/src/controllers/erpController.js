  ...
  try {
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      const uploadStream = cloudinary.uploader.upload_stream(
        { folder: 'company_documents', resource_type: 'auto', type: 'authenticated' },
        (error, result) => {
...
exports.getRecurringTasks = async (req, res, next) => {
  try {
    ...
    const formatted = tasks.map(t => ({
      ...
      nextOccurrence: t.nextOccurrence ? t.nextOccurrence.toISOString().split('T')[0] : 'N/A',
...
exports.getDocumentAccessUrl = async (req, res, next) => {
  try {
    ...
    if (!doc || doc.isArchived) {
      return res.status(404).json({ success: false, message: 'Document not found or archived' });
    }
    ...
    const hasPerm = req.user.permissions?.includes(requiredPerm) || req.user.role === 'admin';
    if (!hasPerm) {
      return res.status(403).json({ success: false, message: 'Forbidden' });
    }

    const version = doc.versions[0];
    if (!version) return res.status(404).json({ success: false, message: 'No versions found' });
