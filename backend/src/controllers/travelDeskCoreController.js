...
const checkManageAccess = (roleOrUser) => {
  if (
    !hasPermission(roleOrUser, "trips.edit") &&
    !hasPermission(roleOrUser, "ops.manage")
  ) {
  ...
  try {
    const trips = await prisma.trip.findMany({
      ...
      select: {
        ...
        tripType: true,
        travelDeskWorkspace: {
          select: { id: true, readinessScore: true, status: true },
        },
      },
      orderBy: { order: "asc" },
    });
...
exports.feedWorkspaces = async (req, res, next) => {
  try {
    ...
    if (!tripIds || !Array.isArray(tripIds)) {
      return res
        .status(400)
        .json({ success: false, message: "tripIds array is required" });
    }
    ...
    for (const tripId of tripIds) {
      const existing = await prisma.travelDeskWorkspace.findUnique({
        where: { tripId },
      });
      if (existing) {
  ...
  try {
    ...
    if (!trip || (!trip.itinerary && !trip.itineraryVersions)) {
      return res
        ...
        .json({
          success: false,
          message: "No master itinerary found for this trip.",
        });
    }
...
exports.getDepartures = async (req, res, next) => {
  try {
    ...
    const trip = await prisma.trip
      .findUnique({ where: { id: tripId }, select: { maxGroupSize: true } })
      .catch(() => null);
    ...
    const departuresList = Object.values(departuresMap).sort((a, b) =>
...
exports.linkVendor = async (req, res, next) => {
  try {
    ...
    if (departureDate && !dateRegex.test(departureDate)) {
      return res
        .status(400)
        .json({ success: false, message: "departureDate must be YYYY-MM-DD" });
    }
    ...
    if (existing) {
      return res
        ...
        .json({
          success: false,
          message: "Vendor already linked for this departure/trip",
        });
    }
...
exports.unlinkVendor = async (req, res, next) => {
  try {
    ...
    const link = await prisma.travelDeskVendorLink.findUnique({
      where: { id: linkId },
    });
    if (!link)
      return res
        .status(404)
        .json({ success: false, message: "Vendor link not found" });
