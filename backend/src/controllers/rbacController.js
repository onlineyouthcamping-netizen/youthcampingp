...
async function logRbacAudit({ tenantId = 'default', userId, userName, userEmail, action, resourceType, resourceId, details, oldValue, newValue, req }) {
  try {
...
exports.createRole = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ success: true, data: newRole, message: `Role "${newRole.name}" created successfully` });
  } catch (err) {
    next(err);
  }
};
...
exports.updateRole = async (req, res, next) => {
  try {
    ...
    if (existing.isSystem) {
      return res.status(403).json({ success: false, message: 'System roles are immutable and cannot be modified' });
    }
    ...
    await logRbacAudit({
      ...
      oldValue: { name: existing.name, permissionCount: existing.permissions.length },
      newValue: { name: updated.name, permissionCount: permissionIds?.length ?? existing.permissions.length },
      req
    });

    res.json({ success: true, data: updated, message: `Role "${updated.name}" updated successfully` });
  } catch (err) {
    next(err);
  }
};
...
exports.cloneRole = async (req, res, next) => {
  try {
    ...
    if (existing) {
      return res.status(400).json({ success: false, message: `Role name "${targetName}" already exists` });
    }
    ...
    res.status(201).json({ success: true, data: cloned, message: `Role cloned successfully as "${cloned.name}"` });
  } catch (err) {
    next(err);
  }
};
...
exports.getPermissionMatrix = async (req, res, next) => {
  try {
    ...
    const matrix = permissions.map(perm => {
      const roleMap = {};
      roles.forEach(role => {
        roleMap[role.id] = role.permissions.some(rp => rp.permissionId === perm.id);
      });
      return {
...
exports.updateUserRoles = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(roleIds) || roleIds.length === 0) {
      return res.status(400).json({ success: false, message: 'At least one role must be assigned' });
    }
    ...
    const primaryRoleObj = await prisma.role.findUnique({ where: { id: targetPrimary } });
    if (primaryRoleObj) {
      const mappedEnum = primaryRoleObj.name.toLowerCase().replace(/\s+/g, '_');
      await prisma.admin.updateMany({
        where: { id: userId },
        data: {
          role: ['super_admin', 'superadmin'].includes(mappedEnum) ? 'superadmin' :
                ['admin', 'sales', 'operations', 'finance', 'guide', 'viewer'].includes(mappedEnum) ? mappedEnum : 'admin'
        }
      });
    }
...
exports.setUserCustomPermission = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: override, message: 'Permission override saved successfully' });
  } catch (err) {
    next(err);
  }
};
...
exports.removeUserCustomPermission = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, message: 'Permission override removed successfully' });
  } catch (err) {
    next(err);
  }
};
...
exports.delegatePermission = async (req, res, next) => {
  try {
    ...
    if (!toUserId || !permissionId || !expiresAt) {
      return res.status(400).json({ success: false, message: 'toUserId, permissionId, and expiresAt are required' });
    }
    ...
    res.status(201).json({ success: true, data: delegation, message: 'Permission delegated successfully' });
  } catch (err) {
    next(err);
  }
};
...
exports.revokeDelegation = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: delegation, message: 'Delegation revoked successfully' });
  } catch (err) {
    next(err);
  }
};
