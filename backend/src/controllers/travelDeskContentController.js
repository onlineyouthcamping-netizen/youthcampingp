...
const { createAuditLog, calculateReadiness } = require('./travelDeskCoreController');
...
exports.getArticles = async (req, res, next) => {
  try {
    ...
    if (!['superadmin', 'admin', 'operations', 'ticketing'].includes(req.user.role)) {
...
exports.createArticle = async (req, res, next) => {
  try {
    const { tripId } = req.params;
    const { categoryId, title, summary, content, visibility, effectiveFrom, expiresAt, tags, originLearningId } = req.body;
    ...
    await createAuditLog(workspace.id, 'ARTICLE', article.id, 'CREATE', null, { title, status: 'DRAFT' }, req.user.id);
...
exports.updateArticle = async (req, res, next) => {
  try {
    ...
    // Unpacking payload
    const { title, summary, content, visibility, effectiveFrom, expiresAt, tags, status } = req.body;
    ...
    if (oldArticle.status === 'PUBLISHED' && newStatus !== 'PUBLISHED' && newStatus !== 'ARCHIVED') {
    ...
    const updated = await prisma.travelDeskArticle.update({
      ...
      data: {
        ...
        effectiveFrom: effectiveFrom !== undefined ? (effectiveFrom ? new Date(effectiveFrom) : null) : oldArticle.effectiveFrom,
        expiresAt: expiresAt !== undefined ? (expiresAt ? new Date(expiresAt) : null) : oldArticle.expiresAt,
    ...
    await createAuditLog(workspace.id, 'ARTICLE', articleId, 'UPDATE', { version: oldArticle.version, status: oldArticle.status }, { version: updated.version, status: updated.status }, req.user.id);
...
exports.requestChangesArticle = async (req, res, next) => {
  try {
    ...
    if (!comment || comment.trim() === '') {
      return res.status(400).json({ success: false, message: 'Mandatory comment required for requesting changes' });
    }

    const article = await prisma.travelDeskArticle.findUnique({ where: { id: articleId } });
    if (!article) return res.status(404).json({ success: false, message: 'Article not found' });
    
    if (article.status !== 'UNDER_REVIEW') {
       return res.status(400).json({ success: false, message: 'Only articles UNDER_REVIEW can have changes requested.' });
    }
    ...
    const workspace = await prisma.travelDeskWorkspace.findUnique({ where: { tripId } });
    await createAuditLog(workspace.id, 'ARTICLE', articleId, 'CHANGES_REQUESTED', { status: 'UNDER_REVIEW' }, { status: 'CHANGES_REQUESTED', comment }, req.user.id);
...
exports.changeArticleStatus = async (req, res, next) => {
  try {
    ...
    if (current === 'CHANGES_REQUESTED' && status === 'UNDER_REVIEW') allowed = true;
    ...
    if (!allowed) {
      return res.status(400).json({ success: false, message: `Invalid transition from ${current} to ${status}` });
    }
    ...
    const workspace = await prisma.travelDeskWorkspace.findUnique({ where: { tripId } });
    await createAuditLog(workspace.id, 'ARTICLE', articleId, `STATUS_${status}`, { status: current }, { status }, req.user.id);
    await calculateReadiness(workspace.id, tripId);
...
exports.getPendingApprovals = async (req, res, next) => {
  try {
    ...
    const mappedArticles = articles.map(a => {
      a.effectiveStatus = (a.expiresAt && a.expiresAt <= now) ? 'EXPIRED' : a.status;
      return a;
    });
...
exports.acknowledgeNotice = async (req, res, next) => {
  try {
    ...
    if (existing) {
       return res.json({ success: true, data: existing, message: 'Already acknowledged' });
    }
