  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(10)
        .font("Helvetica")
        .text(`Invoice No: ${booking.bookingId || "N/A"}`, 400, 75, {
          align: "right",
        })
        .text(`Date: ${new Date().toLocaleDateString("en-IN")}`, 400, 90, {
          align: "right",
        })
        .text(
      ...
      doc
        .fillColor(brandColor)
        .font("Helvetica-Bold")
        .text(booking.tripName || booking.tripId || "N/A", 380, travelY, {
          width: 170,
        });
      ...
      doc.fillColor(brandColor).text(roomType, 380, travelY, { width: 170 });
      ...
      const gstRate =
        booking.baseAmount && booking.gstAmount
          ? booking.gstAmount / booking.baseAmount
          : 0.05;
      const computedBase =
        baseItems.length > 0
          ? basePrice
          : booking.baseAmount ||
            Math.round((booking.totalAmount || 0) / (1 + gstRate));
      const computedGst =
        booking.gstAmount !== null && booking.gstAmount !== undefined
          ? booking.gstAmount
          : Math.round(computedBase * gstRate);
      ...
      if (baseItems.length > 0) {
        baseItems.forEach((item, idx) => {
          ...
          doc
            .font("Helvetica-Bold")
            .text(
              `INR ${(item.rate * item.qty).toLocaleString("en-IN")}`,
              480,
              currentY,
              { align: "right" },
            );
        ...
        doc
          .font("Helvetica-Bold")
          .text(`INR ${computedBase.toLocaleString("en-IN")}`, 480, currentY, {
            align: "right",
          });
      ...
      if (discountItems.length > 0) {
        discountItems.forEach((item) => {
          ...
          doc.font("Helvetica").text(`${item.qty}`, 350, currentY);
          doc
            .font("Helvetica-Bold")
            .text(
              `- INR ${Math.abs(item.rate * item.qty).toLocaleString("en-IN")}`,
              480,
              currentY,
              { align: "right" },
            );
      ...
      doc.moveTo(50, lineY).lineTo(550, lineY).strokeColor("#f1f5f9").stroke();
      ...
      doc
        ...
        .fillColor(brandColor)
        .text(`INR ${computedBase.toLocaleString("en-IN")}`, 450, totalsY, {
          width: 90,
          align: "right",
        });
      ...
      doc
        ...
        .fillColor(brandColor)
        .text(`INR ${computedGst.toLocaleString("en-IN")}`, 450, totalsY, {
          width: 90,
          align: "right",
        });
      ...
      doc
        ...
        .fillColor(brandColor)
        .font("Helvetica-Bold")
        .text(`INR ${computedTotal.toLocaleString("en-IN")}`, 450, totalsY, {
          width: 90,
          align: "right",
        });
      ...
      doc
        ...
        .text(
          `- INR ${(booking.advancePaid || 0).toLocaleString("en-IN")}`,
          450,
          totalsY,
      ...
      doc
        ...
        .fontSize(12)
        .text(
          `INR ${(booking.remainingAmount || 0).toLocaleString("en-IN")}`,
          450,
          totalsY + 12,
          { width: 90, align: "right" },
        );
      ...
      doc
        ...
        .fontSize(8)
        .font("Helvetica")
        .text("Thank you for choosing YouthCamping!", 50, pageHeight - 100, {
          align: "center",
          width: 500,
        })
        .text(
        ...
        .text(
          "Support: support@youthcamping.com | Website: youthcamping.com",
          50,
          pageHeight - 70,
          { align: "center", width: 500 },
        );
