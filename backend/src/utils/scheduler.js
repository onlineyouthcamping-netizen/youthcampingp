    ...
    for (const task of activeTasks) {
      ...
      if (nextRun && nextRun <= now) {
        try {
          ...
          if (err.code === "P2002") {
            console.log(
              `[Scheduler] Task ${task.id} occurrence already exists for ${nextRun}.`,
            );
          } else {
...
async function cleanupPastDates() {
  console.log(
    "[Scheduler] Running past dates cleanup...",
  ...
  try {
    ...
    for (const trip of trips) {
      ...
      if (futureDates.length < originalCount) {
        ...
        console.log(
          `[Scheduler] Trip ${trip.id}: Removed ${originalCount - futureDates.length} past dates.`,
        );
      }
    }
  } catch (err) {
