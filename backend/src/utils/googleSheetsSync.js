...
exports.syncBookingToSheets = async (booking) => {
  ...
  if (!scriptUrl) {
    console.warn(
      "[SHEETS] No GOOGLE_APPS_SCRIPT_URL found in .env, skipping sync",
    );
    return;
  }
  ...
  if (passengers.length > 0) {
    ...
    passengers.forEach((p, idx) => {
      syncItems.push({
        ...
        trainClass:
          booking.passengers?.details?.trainOption ||
          booking.passengers?.details?.trainClass ||
          booking.trainClass,
        ticketStatus:
          booking.passengers?.details?.ticketStatus || booking.ticketStatus,
        roomType:
          booking.passengers?.details?.roomSharing ||
          booking.passengers?.details?.roomType ||
          booking.roomType,
        ...
        notes:
          (booking.notes ||
            booking.adminNotes ||
            booking.passengers?.details?.specialRequests ||
            "") + (idx > 0 ? ` (Part of ${booking.name}'s group)` : ""),
  ...
  try {
    ...
    for (const payload of syncItems) {
      ...
      console.log(
        `[SHEETS] Sync successful for ${payload.fullName}: ${JSON.stringify(response.data)}`,
      );
