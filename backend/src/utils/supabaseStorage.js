...
if (supabaseUrl && supabaseKey) {
  try {
    ...
    console.error(
      "[STORAGE] Failed to initialize Supabase client:",
      err.message,
    );
  }
} else {
...
async function uploadFile(fileBuffer, storagePath, mimeType) {
  ...
  if (supabase) {
    try {
      console.log(
        `[STORAGE] Uploading to Supabase bucket "${BUCKET_NAME}": ${storagePath}`,
      );
      ...
      console.error(
        "[STORAGE] ⚠️ Supabase upload failed, falling back to local:",
        err.message,
      );
    }
  }
...
async function downloadFile(storagePath) {
  ...
  if (supabase) {
    try {
      console.log(
        `[STORAGE] Downloading from Supabase bucket "${BUCKET_NAME}": ${storagePath}`,
      );
      const { data, error } = await supabase.storage
        .from(BUCKET_NAME)
...
async function deleteFile(storagePath) {
  ...
  if (supabase) {
    try {
      console.log(
        `[STORAGE] Deleting from Supabase bucket "${BUCKET_NAME}": ${storagePath}`,
      );
      ...
      if (error) throw error;
      console.log(
        "[STORAGE] ✅ Supabase file deletion successful:",
        storagePath,
      );
      return true;
    } catch (err) {
