...
function makeShutdownHandler(server, prisma) {
  return async (signal, exitCode, error = null) => {
    if (isShuttingDown) {
      console.warn(
        `⚠️ Shutdown already in progress. Ignoring signal/event: ${signal}`,
      );
      return;
    }
    isShuttingDown = true;
    ...
    const forceExitTimeout = setTimeout(() => {
      console.error(
        "🚨 [SHUTDOWN] Force exiting backend server (10s timeout expired)",
      );
    ...
    if (prisma && typeof prisma.$disconnect === "function") {
      try {
        ...
        console.error(
          "❌ [SHUTDOWN] Error during database disconnection:",
          err.message,
        );
      }
    }
