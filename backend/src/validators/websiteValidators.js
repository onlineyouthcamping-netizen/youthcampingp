...
const createPageSchema = z.object({
  ...
  metaTitle: z.string().max(120, 'Meta title must be 120 characters or fewer').optional().nullable(),
  metaDescription: z.string().max(320, 'Meta description must be 320 characters or fewer').optional().nullable(),
  ogImage: z.string().url('OG image must be a valid URL').optional().nullable().or(z.literal('')),
...
const updateSectionSchema = z.object({
  ...
  (data) => data.content !== undefined || data.draft !== undefined || data.name !== undefined || data.visible !== undefined,
  { message: 'At least one field (content, draft, name, visible) must be provided' }
);
