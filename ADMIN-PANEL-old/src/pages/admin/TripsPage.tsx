...
export default function TripsPage() {
  ...
  const openEdit = (t: Trip) => { 
    ...
    }).catch((err: any) => { console.error("Failed to load trip details:", err); toast.error("Failed to load trip details"); });
  };
    ...
    <div className="space-y-6 animate-fade-in p-6 bg-[#F4F7FB] min-h-screen -mx-6 -my-6">
      ...
      <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-4 bg-white -mx-6 -mt-6 p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <GripVertical className="w-4 h-4 text-slate-450" /> Reorder
          </Button>
          <Button 
            onClick={handleShuffle} 
            className="h-8.5 px-3 rounded-[4px] font-semibold text-xs border border-slate-200 bg-white text-slate-650 flex items-center gap-1.5 shadow-xs"
          >
            <Shuffle className="w-4 h-4 text-slate-450" /> Shuffle
          </Button>
          <Button 
            onClick={openCreate} 
