...
export default function TripSortModal({ open, onOpenChange, trips, onSaved }: TripSortModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md sm:max-w-lg p-6 bg-white border border-slate-200 rounded-xl shadow-2xl">
        <DialogHeader className="pr-10 pb-4 border-b border-slate-100 flex flex-col text-left space-y-1">
        ...
        <DialogFooter className="gap-2 sm:gap-0 pt-4 border-t border-slate-100 flex items-center justify-end">
          <Button variant="ghost" className="h-9 px-4 text-xs font-semibold text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={handleSave} disabled={saving} className="h-9 px-5 bg-primary hover:bg-primary/90 text-white text-xs font-semibold rounded min-w-[120px]">
            {saving ? (
              <>
